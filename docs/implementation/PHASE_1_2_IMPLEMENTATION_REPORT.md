...
**Test Results**:
```bash
$ curl -s http://REDACTED_IP:8043/api/v1/array/parity-check/history | jq
{
  "records": [],
...
```bash
# Disk endpoint
$ curl -s http://REDACTED_IP:8043/api/v1/disks/sdb | jq '{id, device, name, role}'
{
...
# Docker endpoint
$ curl -s http://REDACTED_IP:8043/api/v1/docker/jackett | jq '{id, name, state}'
{
  "id": "fedcb3e1ba1f",
  "name": "jackett",
  "state": "running"
...
$ curl -s http://REDACTED_IP:8043/api/v1/vm/test-vm | jq
{
  "success": false,
...
**Test Results**:
```bash
$ curl -s http://REDACTED_IP:8043/api/v1/disks | jq '.[0] | {id, name, role, spin_state, serial_number, model}'
{
  "id": "WUH721816ALE6L4_2CGV0URP",
  "device": "sdb",
  "name": "parity",
...
```bash
# System settings
$ curl -s http://REDACTED_IP:8043/api/v1/settings/system | jq
{
  "server_name": "Cube",
...
# Docker settings
$ curl -s http://REDACTED_IP:8043/api/v1/settings/docker | jq
{
  "enabled": true,
...
# Share configuration
$ curl -s http://REDACTED_IP:8043/api/v1/shares/appdata/config | jq
{
  "name": "appdata",
  "allocator": "highwater",
  "floor": "50000000",
...
3. **Deploy**: Uploaded to Unraid server at REDACTED_IP
...
$ curl -s http://REDACTED_IP:8043/api/v1/health | jq
{
  "status": "ok"
}
```
